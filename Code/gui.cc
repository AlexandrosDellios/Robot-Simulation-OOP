...
#include "constante.h"
#define RESPONSE_SAVE 3 //Add different response type for saving
...
void Fenetre::create_boxes()
{
	...
	m_area.set_expand(); // allow the Simulation to expand to the window size
												...
												Gtk::FileChooserDialog* dialog)
{
	...
	switch (response_id)
	{
		case Gtk::ResponseType::OK: //when opening a file
		{
		...
		case RESPONSE_SAVE: //when saving a file
		{
			std::cout << "Save clicked." << std::endl;
			auto filename = dialog->get_file()->get_path();
			if (!Glib::str_has_suffix(filename, ".txt")) filename += ".txt"; //adds extension if not present
...
void MyArea::setFrame(Frame f)
{
	...
	else cout << "incorrect Model framing or window parameters" << std::endl;
} 
