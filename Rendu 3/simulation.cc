	...
	for(size_t i=0; i < p.size(); i++)
	{
		...
		for(size_t j=min; j < neut.size(); j++)
		{
			...
			double time_min = shape::s2d_norm(distance_min) / vtran_max 
				+ (abs(convertir_angle_retour(angle_min- neut[min].get_alpha()))
					/ vrot_max);
			...
			double time = shape::s2d_norm(distance) / vtran_max 
				+ (abs(convertir_angle_retour(angle - neut[j].get_alpha()))	
					/ vrot_max);
			if((time  < time_min) and (!neut[j].get_panne())) min = j;
		}
...
void Simulation::move_neutraliseurs()
{
	for(size_t i=0; i < neut.size(); i ++)
	{
		...
		colli_neut_spatial(i);
	}
}

void Simulation:: colli_neut_spatial(size_t& i)
{
...
void Simulation::move_reparateurs()
{
	for(size_t i=0; i < rep.size(); i ++)
	{
		...
		if(detect_colli(copie))
		{
			...
			if((shape::colli_cercle(neut[j].get_cercle(),rep[i].get_cercle()
				,false)) and (neut[j].get_panne()))
			{
...
bool Simulation::colli_neut(Robot robot)
{
	...
	for (size_t i(0); i < neut.size(); ++i)
	{
		...
		else if(shape::colli_cercle(robot.get_cercle(), neut[i].get_cercle()
			, false))
		{
			return true;
		}
...
bool Simulation::colli_rep(Robot robot)
{
	...
	for (size_t i(0); i<rep.size(); ++i)
	{
		...
		else if(shape::colli_cercle(robot.get_cercle(), rep[i].get_cercle()
			, false))
		{
			return true;
		}
...
bool Simulation::colli_parti(Robot robot)
{
	...
	{
		if(shape::colli_carre_cercle(parti[i].get_carre(),robot.get_cercle()
			, false))
		{
			return true;
		}
