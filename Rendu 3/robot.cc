...
void Reparateur::move_to(S2d goal)
{
	...
	else shape::s2d_add_scaled_vector(cercle.C, pos_to_goal
		, vtran_max/norm*delta_t);
}
...
//conversion à un angle entre -pi et pi
void converti_angle(double& a)
{
	if(a>M_PI)	a=a-2*M_PI;
	else if (a<-M_PI)	a=a+2*M_PI;
}

double convertir_angle_retour(double a)
{
	if(a>M_PI)	a=a-2*M_PI;
	else if (a<-M_PI) a=a+2*M_PI;
	return a;
